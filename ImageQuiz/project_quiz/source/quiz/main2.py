...
class End(QMainWindow, form_class2):
	...
	def restartGame(self):
		...
		if (recog == 200 and widget.currentIndex() == 2): #버튼입력 and "끝" 화면 상태 
	...
	def buttonClicked(self):
		...
		if self.stopbutton == 0:
			...
			if (recog == 200 and widget.currentIndex() == 1 and self.quizStart != 0): #버튼입력 and 2번째 UI and 퀴즈 준비화면 상태 
				...
				#음성입력
				while answerCount>0:
					ttsList.append(tts.getVoice2Text().find(answerList[self.key].replace(" ","")))
					answerCount -= 1
